...
public class OrderEndpoint implements com.zufar.icedlatte.openapi.order.api.OrdersApi{
    ...
    @Override
    @PostMapping
    public ResponseEntity<OrderResponseDto> addOrder(final OrderRequestDto request) {
        log.info("Received request to add order.");
        var order = manager.addOrder(request);
    ...
    public ResponseEntity<List<OrderResponseDto>> getOrders(final List<OrderStatus> statusList) {
        ...
        var lisOfOrders = manager.getOrders(statusList);
