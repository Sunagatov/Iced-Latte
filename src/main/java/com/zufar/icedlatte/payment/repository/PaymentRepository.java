...
public interface PaymentRepository extends JpaRepository<Payment, UUID> {
    ...
    /**
     * Row-level lock to prevent concurrent webhook processing for the same order. Different Stripe events (e.g.,
     * completed + async_payment_succeeded) can target the same order with different event IDs — event dedup alone is
     * not enough.
     */
