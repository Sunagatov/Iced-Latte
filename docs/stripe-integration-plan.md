...
Iced Latte is a **source-available educational project** created for learning and gaining senior software engineering experience. This Stripe integration uses **Stripe test mode / sandbox only** — no real cards, no real money. Stripe Hosted Checkout in test mode handles all card UI; Iced Latte never sees or stores card details.
